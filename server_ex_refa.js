...
const DATABASE = {
  ...
  QUERY: {
    STUDENTS: {
      ...
      UPDATENAMEBYID: "UPDATE STUDENTS SET` NAME = ? WHERE ID = ?",
    },
  },
};
...
const upDateNameStudents = async (id, name) =>
  await executeQuery(DATABASE.QUERY.STUDENTS.FINDBYADDRESS, [name, id]);
...
app.post("/api/student/rename", async (req, res) => {
  ...
  const result = await upDateNameStudents(id, name);
  res.json(result);
});
